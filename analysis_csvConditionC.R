...
do_expmdat_binary4_csv = function(res,generated_stims, accthreshold, alsoburnin=F, dyadsuffix=NA, doinfandcompl=F){
  ...
  for(i in ok){
    ...
    meanings = generated_stims[[3]]$words
    stimtargets = generated_stims[[3]]$targets
...
do_expmdat_fromfile_csv = function(
  ...
  res <- lapply(res, function(x) {x %>% rename(pair1 = word1, pair2 = word2, stimn= iterationNum,sender=speakerID, say =intendedName, sent=topSpeakerChoice, guess=topListenerChoice)})
  res <- lapply(res, function(x) { x %>%   mutate(burnin = case_when(
    time > 40 ~ FALSE,
    time <= 40 ~ TRUE
  ))})
