...
stimcombinator = function(stims, params, actualtargets=NULL){
  ...
  isbaseline = c(rbind(  rep("b0", params$npairspercond),
                         rep("b1", params$npairspercond),
                         rep("t0", params$npairspercond),
                         rep("t1", params$npairspercond),
                         rep("c0", params$npairspercond),
                         rep("c1", params$npairspercond)
  ...
  for(s in seq_along(stims$words)){
    ...
    if(isbaseline[s] == "b0"){
    ...
    } else if (isbaseline[s] == "b1") {
      pairs = c(rep(targs, each=pm["baseline"]), 
                rep(others, each=pm["baseline"] ), 
                rep(halftargs, each=pm["baseline"] ) 
      )
    }
      else if (isbaseline[s] == "t0") {
      ...
      } else if (isbaseline[s] == "t1") {
        pairs = c(rep(targs, each=pm["targetsCond"]), 
                  rep(others, each=pm["distractorsCond"] ), 
                  rep(halftargsoften, each=pm["halfbigtargetsCond"]),
                  rep(halftargsfew, each=pm["halfsmalltargetsCond"])
        )
      } else if (isbaseline[s] == "c0") {
    ...
    while(sdx>3){  # not this might need adjusting if different sample size, will not try forever:
      ...
      for(i in which(!pairs2$burnin)){
        ...
        pairs2$say[i] = x
        freqMultiplier = 1
        if (x %in% twordsfew && (isbaseline[s] == "t1" || isbaseline[s] == "t0")) {
          freqMultiplier = 3
        }
...
params = list(
  ...
  npairspercond     = 40    , # how many stims sets (out of nstims) are in the baseline/control condition
