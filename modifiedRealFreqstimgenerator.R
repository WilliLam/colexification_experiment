...
stimcombinator = function(stims, params, actualtargets=NULL){
  ...
  isbaseline = c(rbind(  rep(1, params$nstims-params$nbaseline),
                         rep(2, params$nbaseline),
                         rep(3, params$nbaseline)
  )) # assumes equal n
  for(s in seq_along(stims$words)){
    ...
    #
    nx = 0; sdx=Inf
    while(sdx>6){  # not this might need adjusting if different sample size, will not try forever:
